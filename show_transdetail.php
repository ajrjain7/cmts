...
{
show_sp_transaction($_GET['trans_id']);
show_detail_sptrans($_GET['trans_id']);
}
else '<h2>Invalid Cust/Trans ID</h2>';
//ELSE REDIRECT

function show_sp_transaction($trans_id)
{
echo '>>>Customer ID: '.$_GET['custid'].'  >>>Trans ID: '.$trans_id.'
<h3>Sale Purchase Firm Transaction</h3>
<table class="" border=1 style="background-color:white;font-size:17px;text-align:center;color:black">
		...
		<th width="15%">Firm</th>
		<th width="10%">Item</th>
		<th width="10%">Total Bill<hr>Paid</th>
...
try{ 	
			...
			$firesql="select * from sp_transaction where trans_id=".$_GET['trans_id']; //TAKE INNER JOIN OF CUST_INFO AND M_TRANSACTION
...
echo '<td>'.$row['firm_name'].'</td>';
   echo '<td>'.$row['item_purchased'].'</td>';
   			echo '<td>'.$row['total_bill'].'<hr>'.$row['Paid'].'</td>';		
				echo '<td style="color:red">'.($row['total_bill']-$row['Paid']).'</td>';		
				echo '<td style="color:red">'.($row['Interest_Amt_Due']+ round(($row['total_bill']-$row['Paid'])*(float)($time)*(float)$row['Interest']/36500)).'</td>';		
...
function show_detail_sptrans($trans_id)
{
echo '
<h3>Detail</h3>
<table class="zebra">
...
try{ 	
			...
			$firesql="select * from sp_trans_detail where trans_id=".$trans_id." order by sys_creation_date"; //TAKE INNER JOIN OF CUST_INFO AND M_TRANSACTION
			$res=mysqli_query($con1,$firesql);
			while($row=mysqli_fetch_array($res))
			{				
				echo '<tr><td style="color:blue"><a href=./update_trans_detail?trans_id='.$row['trans_id'].'&date='.$row['date'].'>'.$row['trans_id'].'</a></td>';
				echo '<td>'.$row['amt'].'</td>';			
				echo '<td>'.$row['comments'].'</td>';	
				echo '<td>'.$row['date'].'</td>';			
				echo '</td></tr>';				
...
<br />
<a href=./update_trans.php?trans_id='.$trans_id.'><input type="button" value="Add New Payment Entry" style="font-size:18px;font-family:BOLD">
';
}
